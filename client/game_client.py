...
class PcrClient:
    ...
    # 调用接口
    async def callapi(self, api_url: str, request: dict):
        # 补充请求和headers的参数
        ...
        self.headers['PARAM'] = sha1(
            (self.udid + api_url + b64encode(packed).decode('utf8') + str(self.viewer_id)).encode('utf8')).hexdigest()
        self.headers['SHORT-UDID'] = PcrClient._encode(self.short_udid)

        # 发出请求
        logger.info(f'[PcrClient] 请求接口 {api_url} ')
        resp = await self.async_session.post(
            self.api_root + api_url,
            data=crypto,
