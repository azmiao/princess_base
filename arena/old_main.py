...
from ..util import gadget_path

# 点赞图片
thumb_up_a = Image.open(os.path.join(gadget_path, 'thumb-up-a.png')).resize((16, 16), Resampling.LANCZOS)
thumb_down_a = Image.open(os.path.join(gadget_path, 'thumb-down-a.png')).resize((16, 16), Resampling.LANCZOS)
...
dataDir = join(cur_path, 'dic.npy')
if not exists(dataDir):
    update_dic()
data = np.load(dataDir, allow_pickle=True).item()
data_processed: Optional[dict] = None

best_atk_records_path = join(cur_path, 'buffer', 'best_atk_records.json')
...
async def cut_image(image, hash_size=16):
    ...
    image1 = image.resize((hash_size + 1, hash_size), Resampling.LANCZOS).convert('L')
...
async def calc_distance_img(image1, image2):
    hash_arr1 = await get_hash_arr(image1)
    hash_arr2 = await get_hash_arr(image2)
    return await calc_distance_arr(hash_arr1, hash_arr2)
...
async def cutting(img, mode):
    ...
    :param mode: mode=1：返回图片中最大的长方形区域，以及该区域的定位点 [PIL.Image, [x, y, w, h]] ; mode=2: 返回两个列表，
    ...
    contours, _ = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)  # 获取轮廓
    img2 = thresh
    ...
    if mode == 2:
        ...
        for i in icon:
            ...
            for kind in kinds:
                ratio = side_len / kind
                if 0.9 < ratio < 1.1:  # 将边长相差10%以内作为一类
                    category = kind
        ...
        def clusterWeight(_x):
            _side_len = _x[0]
            val = len(_x[1])
            if val == 5:  # 该类有5个元素，优先返回。第二关键字为边长（从大到小）。
                return 5000000 + _side_len
            if val % 5 == 0:  # 该类元素个数为5的倍数，次优先返回。
                return 1000000 + _side_len
            return val * 10000 + _side_len
        ...
        if len(kind[1]) % 5 == 0:  # 存在某一类，其元素个数为5的倍数，返回该类
            ...
            for x in range(1, len(kinds)):
                other_border += kinds[x][1]
            return kind[1], other_border
        else:
...
async def getPos(img: Image):
    ...
    :return 识别出的阵容的坐标, 识别出的阵容角色字符串 [[第1队第1个角色(int), 1-2, ..., 1-5], [2-1, ..., 2-5], [3-1, ..., 3-5]],
     str；若识别失败，返回[], ""
    """
    ...
    now_color = 0
    outImg = Image.new(mode="RGBA", size=img.size, color=ImageColor.getrgb(f'rgb({now_color},{now_color},{now_color})'))
    outImgText = Image.new("RGBA", img.size, (0, 0, 0, 0))
    outImgTextDraw = ImageDraw.Draw(outImgText)

    cnt = 0
    while cnt <= 6:
        bo = False
        cnt += 1
        border, other_border = await cutting(im_grey, 2)  # 获取当前图像中的正方形区域
        if len(border) == 0:  # 如果莫得 说明需要翻转图像的黑白
            ...
            def highlight(rec, color="red"):  # 识别成功red 被其它代码排除的识别项blue 超过五列未识别的green 识别不出角色的black 识别出是100031的yellow
                _x, _y, _w, _h = rec
                _cropped = img.crop((_x + 2, _y + 2, _x + _w - 2, _y + _h - 2))
                outImgText.paste(_cropped,
                                 (actual_x + _x + 2, actual_y + _y + 2, actual_x + _x + _w - 2, actual_y + _y + _h - 2))
                outImgTextDraw.rectangle((actual_x + _x, actual_y + _y, actual_x + _x + _w, actual_y + _y + _h),
                                         fill=None,
                                         outline=color,
                                         width=4)

            for i in other_border:
                highlight(i, 'blue')
            if len(border) < 4:  # 由5改为4 是为了日后准备加入残缺队伍查询（通过无api版本）
                ...
                def split_last_col_recs(_recs):
                    _recs = sorted(_recs, key=lambda _x: _x[0], reverse=True)
                    _last_col_recs = [rec for rec in _recs if abs(rec[0] - _recs[0][0]) < _recs[0][2] / 2]
                    _last_col_recs = sorted(_last_col_recs, key=lambda _x: _x[1])
                    return list(set(_recs) - set(_last_col_recs)), _last_col_recs
                ...
                # 创建一个 row 行 5列 的画布，行间及四周留16px空隙，每行中的每列分为上下两个头像：截出来的和通过识别的id render出来的（均为64*64)。头像间隙0px。
                icon_size = 64
                ...
                for row_index in range(row_cnt):
                    ...
                    for col_index in range(5):
                        ...
                        cropped = (img.crop((x + 2, y + 2, x + w - 2, y + h - 2))
                                   .resize((64, 64), Resampling.LANCZOS))
                ...
                outImg = Image.blend(outImg, actual_img, 0.2)
                outImg.alpha_composite(outImgText)
                ratio = max(1, max(outImg.size[0], outImg.size[1]) // 500)
                outImg = outImg.resize((int(outImg.size[0] / ratio), int(outImg.size[1] / ratio)),
                                       Resampling.LANCZOS)

                return arr_id, f'{out_b64(outImg)}\n{out_b64(compare_img)}'
        ...
        if cnt == 1 or bo:  # 如果是第一次裁剪，或者识别不到正方形区域，反色
            im_grey = im_grey.point(lambda _x: 0 if _x > 128 else 255)
        img = await cut(img, border)  # 将原始img也裁剪，和im_grey同步
        actual_x += border[0]
        actual_y += border[1]

        now_color = (now_color + 60) % 300
        outImg.paste(ImageColor.getrgb(f'rgb({now_color},{now_color},{now_color})'),
...
async def getUnit(img2):
    img2 = img2.convert("RGB").resize((128, 128), Resampling.LANCZOS)
    img3 = np.array(img2)
...
async def generateCollisionFreeTeam(bot, ev, all_query_records, team_has_result, region, boxDict):
    """
    all_query_records
    [
        [
...
def remove_buffer(uid: str):
    with open(buffer_json_path, 'r', encoding="utf-8") as _fp:
        buffer = json.load(_fp)

    try:
        remove(join(cur_path, 'buffer', f'{uid}.json'))
    except:
        pass

    if uid in buffer:
        buffer.pop(uid)
        with open(buffer_json_path, 'w', encoding="utf-8") as _fp:
            # noinspection PyTypeChecker
            json.dump(buffer, _fp, ensure_ascii=False, indent=4)


async def _QueryArenaTextAsync(text: str, region: int, bot, ev):
    defense = re.sub(r'[?？，,_]', '', text)
...
async def __arena_query(bot, ev, region: int, defense, raw=0, only_use_cache=False):
    ...
    defense = [chara.get_chara_by_id(x).name for x in defense]
    defense = f"防守方【{' '.join(defense)}】"
...
async def _update_dic_cron():
    ...
    try:
        global best_atk_records
        update_record()
        with open(best_atk_records_path, "r", encoding="utf-8") as _fp:
            best_atk_records = json.load(_fp)
    except:
        pass
